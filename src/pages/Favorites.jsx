  ...
  const filterQuizzes = () => {
    return favoriteQuizzes.filter((q) => {
      const levelFilter =
        ...
        activeFilters.levels.includes(q.level);
      const categoryFilter =
        activeFilters.categories.length === 0 ||
        activeFilters.categories.includes(q.category);
      const labelsFilter =
        activeFilters.labels.length === 0 ||
        activeFilters.labels.some((label) => q.labels.includes(label));
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        ...
        >
          ...
          {filteredQuizzes.length === 0 && (
            ...
            >
              Save your favorite quizzes so they are here.
            </Typography>
          )}
          {filteredQuizzes.length > 0 && (
            <Box sx={{ width: '100%' }}>
